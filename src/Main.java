...
public class Main {
    ...
    private void launch(String... args) {
        ...
        try (InputStream is = new FileInputStream(file)) {
            ...
            APTA aptaOriginal = new APTA(is);
            logger.info("APTA was successfully built");

            logger.info("APTA size: " + aptaOriginal.getSize());
            logger.info("Ends in APTA: " + (aptaOriginal.getAcceptableNodes().size() + aptaOriginal.getRejectableNodes().size()));
            logger.info("Count of words: " + aptaOriginal.getCountOfWords());

            int bestSolution = -1;
            for (int run = 1; run <= Settings.RUNS; run++) {

                logger.info("==================================");
                logger.info("Run: " + run + "/" + Settings.RUNS);

                APTA apta = new APTA(aptaOriginal);
//                logger.info("APTA was successfully built");
//
...
//                logger.info("Count of words: " + apta.getCountOfWords());
                ...
                if (!backtrackingMode) {
                    ...
                    if (!noisyMode) {
                        ...
                        Settings.MINIMUM_SIZE = Math.max(cliqueSize, Settings.MINIMUM_SIZE) + Settings.OFFSET;
                        if (bestSolution != -1) {
                            Settings.MINIMUM_SIZE = Math.min(Settings.MINIMUM_SIZE, bestSolution);
                        }
                        if (Settings.OFFSET != 0) {
                            Settings.MAXIMUM_SIZE = Settings.MINIMUM_SIZE;
                        }
                        minSize = Settings.MINIMUM_SIZE;
                        maxSize = Settings.MAXIMUM_SIZE;
                        ...
                        try {
                            ...
                            do {
                                ...
                                if (solver.problemIsSatisfiable()) {
                                    ...
                                    if (bestSolution == -1 || colors < bestSolution) {
                                        bestSolution = colors;
                                        logger.info("Size " + colors + " is the best new solution!");
                                    }
                                    int[] model = null;
                                    try {
                                    ...
                                    } else if (Settings.RUNS > 1) {
                                        fullResultFilePath += fineNumber(run);
                                    }
