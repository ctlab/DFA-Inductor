			...
			if (!backtrackingMode) {
				...
				if (!noisyMode) {
					...
					if (Settings.OFFSET > 0) {
						from = Math.max(cliqueSize, Settings.MINIMUM_SIZE) + Settings.OFFSET;
						if (bestSolution != -1) {
							if (bestSolution < from - Settings.OFFSET) {
								continue;
							}
							from = Math.min(from, bestSolution);
						}
						to = from;
					} else {
						from = Math.max(cliqueSize, Settings.MINIMUM_SIZE);
						to = Settings.MAXIMUM_SIZE;
					}

					logger.info("Clique was found. Its size is " + cliqueSize + ".");
					logger.info("Searching will be started from size " + from + ".");
				}
				boolean found = false;
				for (int colors = from; colors <= to && !found; colors++) {
					...
					try {
						...
						do {
							...
							if (solver.problemIsSatisfiable()) {
								...
								if (findAllMode) {
									fullResultFilePath += "_" + colors + "_" + fineNumber(curDFA);
